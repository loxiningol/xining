...
6. **收益硬度门禁（硬）**：带杠杆后的 **周收益代理 ≥ 8%**，**收益/回撤 ≥ 1.0**；否则换视角重做。因子多空周收益（带杠杆、扣双边成本）**< 3% → 丢弃**（即便 IC 显著）。
7. **策略退化熔断（硬）**：日均暴露 < 10%，或平均单笔净利 < 1bp，或窗内总收益 < 1% → 判定退化，禁止输出，回溯元思考换微观结构视角。
8. **元思考收益锚点**：设计文档必须含 `expected_annual_return_range` 与 `minimum_acceptable_annual_return`；发散时估算每种视角 `max_annual_net_estimate`。
