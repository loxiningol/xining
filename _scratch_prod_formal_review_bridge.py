...
def submit_blueprint_to_formal_review(blueprint, job):
    ...
    with process_lock("formal_review_submission"):
        ...
        # A formal review can correctly reject a candidate.  That is still a
        # real, auditable submission to the review system and must not be
        # misreported as "never submitted" merely because it did not graduate
        # to human approval.
        review_submitted = bool((result or {}).get("task_id"))
        return {
            "ok": bool((result or {}).get("ok")),
            "started": review_submitted,
            "attempted": True,
            "submission_created": review_submitted,
            "human_approval_submission_created": human_approval_submission_created,
            "review_submitted": review_submitted,
            "reason": (result or {}).get("reason"),
            "task_id": (result or {}).get("task_id"),
            "review_stage": (result or {}).get("stage") or (result or {}).get("reason"),
