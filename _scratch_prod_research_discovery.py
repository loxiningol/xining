...
def build_hypothesis_population(brief, symbol, timeframe, factor_matrix, fwd_returns,
    ...
    strict_contract = bool(
        (research_contract or {}).get("family_hints") or contract_features
    )
    focused = []
    excluded_by_focus = []
    for row in annotated:
        ...
        if not strict_contract or named_branch or feature_rediscovery or branch_control:
    ...
    hyps = focused if strict_contract else annotated
...
def run_discovery(
    ...
    if survivors:
        ...
        for rank_index, candidate in enumerate(list(survivors)):
            ...
            # are unchanged.
            confirmation_stats = candidate.get("confirmation_stats") or {}
            evidence.update({
                "oos_confirmation_present": bool(candidate.get("confirmation_returns")),
                "oos_confirmation_passed": bool(
                    int(confirmation_stats.get("n") or 0) >= 8
                    and float(confirmation_stats.get("mean_net") or -1e9) > 0
                    and float(confirmation_stats.get("hac_t_stat") or -1e9) >= 1.0
                    and float(confirmation_stats.get("efr") or -1e9) >= 1.20
                ),
                "oos_n": int(confirmation_stats.get("n") or 0),
    ...
    return {
        ...
        "present_to_assembly": ok,
        "human_banner_zh": (
            "研究发现通过：%d 个假设经裸探针、独立确认、反证、泄漏/因果边界、执行、EFR与Kimi独立裁判后获准提交四次正式复核；"
