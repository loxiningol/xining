#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Submit macro_sfp_displacement to STEP A with L1-seed retries. No --confirm / no mount."""
...
PACK = Path(sys.argv[1] if len(sys.argv) > 1 else "/root/strategy_macro_sfp_displacement_v1.json")
...
MAX_TRIES = int(sys.argv[5] if len(sys.argv) > 5 else 6)
TAG = sys.argv[6] if len(sys.argv) > 6 else "macro_sfp_displacement_v1"
...
def main():
    ...
    print("STEP_A", STEP_A_CODE_VERSION, "submit", SYMBOL, TIMEFRAME, DIRECTION,
    ...
    for i in range(1, MAX_TRIES + 1):
            ...
            result = run_creation_pipeline_step_a(
                ...
                prebuilt_spec_pack={
                    ...
                    "meta": {
                        ...
                        "source": "macro_sfp_displacement_handcraft",
                    },
                    "errors": [],
                    "call_id": "macro_sfp_%s_%s_%d" % (DIRECTION, int(time.time()), i),
                    "attempts": 0,
                },
        ...
        # enrich from nested task if present
        for k in ("gate_results", "phase3_funnel", "pending_path", "production_mounted"):
            if (result or {}).get(k) is not None:
                summary[k] = result.get(k)
        print("SUMMARY", json.dumps(summary, ensure_ascii=False, default=str)[:2000], flush=True)
        ...
        if reason in ("funnel_l1_fail",):
            continue  # seed retry
        # stop on gate2/mechanism/kb etc — redesign needed
        break

    out = ROOT / "auto_trade" / "dual_engine" / "workflow_v2" / (
        "macro_sfp_submit_%s_%s.json" % (DIRECTION, time.strftime("%Y%m%d_%H%M%S"))
    )
