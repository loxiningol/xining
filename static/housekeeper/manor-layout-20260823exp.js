(function(){
  ...
  var specs=[
    {id:'furnace1',kind:'furnace',title:'炼金熔炉一',status:'等待研究方向',x:402,y:320,w:190,h:235,z:40,target:'#atmSectionCreation',focus:'.atm-pipe-card[data-pipeline="1"]'},
    {id:'furnace2',kind:'furnace',title:'炼金熔炉二',status:'等待研究方向',x:580,y:320,w:190,h:235,z:41,target:'#atmSectionCreation',focus:'.atm-pipe-card[data-pipeline="2"]'},
    {id:'quality',kind:'windmill',title:'风车质检工坊',status:'等待策略产出',x:977,y:255,w:230,h:285,z:39,target:'#atmSectionReview',focus:'#atmQiCube'},
    {id:'bakery',kind:'bakery',title:'合格策略面包房',status:'0 份过关',x:1355,y:305,w:260,h:240,z:43,target:'#atmSectionBakery',focus:'#atmBakeryRounds'},
    {id:'experimental',kind:'greenhouse',title:'策略模版温室',status:'模版目录待命',x:520,y:610,w:220,h:215,z:57,target:'#mdqModuleExperimental'},
    {id:'optimizer',kind:'workshop',title:'已有策略模版工坊',status:'工坊待命',x:665,y:615,w:160,h:185,z:58,target:'#atmSectionPendingOptimize',focus:'#atmWorkshopPipelines'},
    {id:'forecast',kind:'board',title:'中央组合告示牌',status:'组合数据读取中',x:960,y:555,w:170,h:135,z:65,target:'#atmSectionForecast'},
    {id:'backtest',kind:'library',title:'记录手动经验记录馆',status:'经验记录待命',x:1215,y:548,w:225,h:205,z:59,target:'#mdqModuleBacktest'},
    {id:'records',kind:'archive',title:'策略档案馆',status:'运行策略读取中',x:435,y:855,w:255,h:230,z:77,target:'#atmSectionStrategies'},
    {id:'timeline',kind:'memorial',title:'庄园纪念馆',status:'系统大事年表',x:840,y:865,w:190,h:175,z:79,target:'#mdqModuleTimeline'},
    {id:'strategies',kind:'manor',title:'自动交易庄园',status:'实盘庄园区',x:1388,y:880,w:250,h:190,z:82,zoneFocus:'live'},
    // 观风金球 hotspot: center of painted rooftop globe on manor-world-v7 (not the balcony).
    {id:'hold_orb',kind:'orb',title:'观风金球',status:'辅助监测待命',x:1435,y:606,w:40,h:40,z:120,target:'#atmSectionHoldAssist'},
    {id:'pos_greenhouse',kind:'greenhouse',title:'仓位暖房',status:'仓位读取中',x:1608,y:848,w:168,h:158,z:88,target:'#atmSectionPositions'},
    {id:'health',kind:'sign',title:'核心服务木牌',status:'服务状态读取中',x:1516,y:502,w:78,h:96,z:72,target:'#mdqModuleHealth'},
    {id:'actions',kind:'sign',title:'系统操作木牌',status:'系统操作 · 安全网',x:1600,y:528,w:78,h:96,z:73,target:'#mdqModuleOpsSign'},
    {id:'period',kind:'sign',title:'每日/周情况汇总',status:'日周汇总读取中',x:1688,y:554,w:84,h:96,z:74,target:'#atmSectionPeriodSummary'},
    {id:'market',kind:'tower',title:'盘面观测塔',status:'盘面读取中',x:1810,y:500,w:190,h:285,z:70,target:'#mdqModuleMarket'}
  ];

  var zones=[
    {id:'alchemy',label:'策略炼金区',short:'炼金区',x:335,y:120,cx:650,cy:330,scale:.82},
    {id:'craft',label:'成果工坊区',short:'成果区',x:1335,y:120,cx:1230,cy:410,scale:.82},
    {id:'live',label:'实盘庄园区',short:'实盘区',x:1660,y:910,cx:1510,cy:670,scale:.82},
    {id:'archive',label:'研究档案区',short:'档案区',x:330,y:970,cx:600,cy:730,scale:.82}
  ];
  ...
  function createZones(){
    zones.forEach(function(zone){
      ...
      button.addEventListener('click',function(){focusArea(zone)});
  ...
  function createBuilding(spec){
    ...
    button.addEventListener('click',function(event){
      ...
      clearBuildingReveal();
      if(spec.zoneFocus){
        var zone=zones.filter(function(item){return item.id===spec.zoneFocus})[0];
  ...
  function createDepthScene(){
    ...
    [
      ...
      ['bakery',1310,350],['manor',1405,900],['hold_orb',1435,606],
      ['pos_greenhouse',1600,880],['archive',486,832],['gazebo',840,900],['workshop',678,568]
  ...
  function createControls(){
    ...
    root.appendChild(el('div','mdq-manor-legend','悬停建筑查看标题 · 手机轻点查看、再点进入 · 拖动/缩放游览'));
  }
  ...
  var pipeLiveOverride={1:false,2:false};
  var pipeLiveHeard=false;

  function furnaceBusyText(text){
    text=String(text||'');
    if(/状态获取失败/.test(text))return false;
    return /正在工作|工作中|研究中|在研|计算中|处理中/.test(text);
  }
  ...
  function syncFurnaceLive(){
    if(pipeLiveHeard){
      applyFurnaceLive(pipeLiveOverride[1],pipeLiveOverride[2]);
      return;
    }
    applyFurnaceLive(furnaceBusyText(textOf('atmPipe1State')),furnaceBusyText(textOf('atmPipe2State')));
  }
  ...
  function syncStates(){
    ...
    [1,2].forEach(function(number){
      ...
      pipeText+=' '+state;
      setBuilding('furnace'+number,inferState(state),[state,pct].filter(Boolean).join(' · ')||'等待研究方向');
    });
    var pipeBusy=/正在工作|工作中|研究中/.test(pipeText)&&!/状态获取失败/.test(pipeText);
    root.classList.toggle('has-creation-flow',pipeBusy);
    syncFurnaceLive();

    var qi=textOf('atmQiCount');
    var review=textOf('atmReviewStatus');
    setBuilding('quality',inferState(review),qi||'等待策略产出');
    root.classList.toggle('has-quality-work',/运行|质检中|处理中/.test(review));
    setBuilding('bakery','ok',textOf('atmBakeryCount')||'0 份过关');
    var workshopBusy=false;
    [3,4].forEach(function(number){
      var st=textOf('atmPipe'+number+'State');
      if(/正在工作|工作中|研究中/.test(st)&&!/状态获取失败|排队/.test(st))workshopBusy=true;
    });
    var workshopLabel=textOf('atmPendingOptimizeCount')||textOf('atmWorkshopBatch')||'工坊待命';
    setBuilding('optimizer',workshopBusy?'active':inferState(workshopLabel+' '+textOf('atmPipe3State')+' '+textOf('atmPipe4State')),workshopLabel);
    root.classList.toggle('has-workshop-flow',workshopBusy);
    setBuilding('forecast',inferState(textOf('systemForecastPanel')),textOf('atmFcGeoWeekly')?('周几何增长率 '+textOf('atmFcGeoWeekly')):'组合数据读取中');
    setBuilding('strategies','ok','实盘庄园区');
    ...
    setBuilding('hold_orb',holdBusy?'active':inferState(textOf('atmHoldAssistRoster')+' '+holdText),holdText||'辅助监测待命');
    ...
    setBuilding('records',inferState(textOf('atmStrategyRoster')),textOf('atmStrategyCount')||'运行策略读取中');
    setBuilding('backtest',inferState(textOf('mdqExpCount')),textOf('mdqExpCount')||'经验记录待命');
    var expText=textOf('experimentalStrategies');
    var expN=(document.querySelectorAll('#experimentalStrategies details')||[]).length;
    var expStatus=expN?(expN+' 份模版'):(/失败/.test(expText)?'模版读取失败':(/正在读取|读取中/.test(expText)?'模版读取中':'模版目录待命'));
    setBuilding('experimental',inferState(expStatus+' '+expText),expStatus);
    setBuilding('actions',inferState(textOf('safetyNetBox')),textOf('safetyNetManorStatus')||'系统操作 · 安全网');
    setBuilding('timeline','ok','系统大事年表');
    setBuilding('market',inferState(textOf('dialysisBox')),textOf('marketMonitorTime')||'盘面读取中');
    var health=textOf('processBox');
    setBuilding('health',inferState(health),/异常|错误|未运行/.test(health)?'检测到服务异常':'核心服务正常');
    var day=textOf('atmDayOpenAuto');
    var week=textOf('atmWkOpenAuto');
    var periodBits=[];
    if(day) periodBits.push('日 '+day);
    if(week) periodBits.push('周 '+week);
    setBuilding('period',inferState(day+' '+week),periodBits.join(' · ')||'日周汇总读取中');
  }

  function observeStates(){
    var targetIds=[
      'atmPipe1State','atmPipe2State','atmPipe1Pct','atmPipe2Pct','atmPipe3State','atmPipe4State',
      'atmQiCount','atmReviewStatus','atmBakeryCount','atmBakeryStatus',
      ...
      'dialysisBox','processBox','safetyNetBox','safetyNetManorStatus','atmDayOpenAuto','atmWkOpenAuto','experimentalStrategies'
    ];
    var queued=false;
    ...
    document.addEventListener('hk:pipelines',function(event){
      ...
      root.classList.toggle('has-creation-flow',pipes.some(function(pipe){
        var id=Number(pipe&&pipe.pipeline);
        return pipe&&pipe.working&&(id===1||id===2);
      }));
      root.classList.toggle('has-quality-work',pipes.some(function(pipe){
        var id=Number(pipe&&pipe.pipeline);
        var stage=String(pipe&&pipe.progress&&pipe.progress.stage||'');
        return pipe&&pipe.working&&(id===1||id===2)&&/quality|blind_oos|handoff/.test(stage);
      }));
      if(actorEngine)actorEngine.onPipelines(event.detail||{});
      pipeLiveOverride={1:false,2:false};
      pipeLiveHeard=true;
      pipes.forEach(function(pipe){
        var id=Number(pipe&&(pipe.pipeline||pipe.pipeline_id)||0);
        if(pipe&&pipe.working&&(id===1||id===2))pipeLiveOverride[id]=true;
      });
      syncStates();
    });
    document.addEventListener('hk:workshop',function(event){
      var detail=event.detail||{};
      var busy=Number(detail.working||0)>0||(Array.isArray(detail.pipelines)&&detail.pipelines.some(function(pipe){return pipe&&pipe.working}));
      root.classList.toggle('has-workshop-flow',busy);
