...
_SCRUB_RULES = [
    ...
    (re.compile(r"review3_fail", re.I), "%s未过：矩阵 / 抗离群" % REVIEW_3),
    (re.compile(r"trades_ge_\d+", re.I), "%s未过：成交笔数不足" % REVIEW_2),
    (re.compile(r"worst5_loss_share[^\s]*", re.I), "%s未过：最差五笔亏损过于集中" % REVIEW_3),
    (re.compile(r"\bR1\b"), REVIEW_1),
    (re.compile(r"\bR2\b"), REVIEW_2),
    (re.compile(r"\bR3\b"), REVIEW_3),
    (re.compile(r"\bR4\b"), REVIEW_4),
